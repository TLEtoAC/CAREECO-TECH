...
class SimpleEnhancedPharmaSearch:
    """Simplified enhanced search without NLTK dependency"""
    ...
    def simple_clean_text(self, text):
        ...
        medical_terms = {'mg', 'tablet', 'capsule', 'injection', 'syrup', 'ml', 'gm'}
        tokens = [token for token in tokens if token not in self.stop_words or token in medical_terms]
    ...
    def prepare_enhanced_search_data(self):
        ...
        self.df['enhanced_search_text'] = (
            self.df['medicine_name'].fillna('').apply(self.simple_clean_text) + ' ' +
            self.df['salt_composition'].fillna('').apply(self.simple_clean_text) + ' ' +
            self.df['marketed_by'].fillna('').apply(self.simple_clean_text) + ' ' +
            self.df['packagingType'].fillna('').apply(self.simple_clean_text)
        )
    ...
    def identify_duplicates(self):
        """Identify duplicate medicines using optimized approach"""
        print("🔄 Identifying duplicate medicines (fast method)...")
    ...
    def enhanced_search(self, query, filters=None, top_k=20):
        """Enhanced search with ranking and duplicate handling"""
        if not query.strip():
            return []
        ...
        # Clean the query
        cleaned_query = self.simple_clean_text(query)
    ...
    def combine_and_rank_results(self, symptom_results, tfidf_results, original_query, top_k):
        ...
        for result in results:
            score_multiplier = self.calculate_relevance_score(result, original_query)
            result['confidence'] = min(100, int(result['confidence'] * score_multiplier))
    ...
    def calculate_relevance_score(self, medicine, query):
        ...
        return min(1.8, score_multiplier)
    ...
    def get_search_analytics(self):
        ...
        return {
            'total_medicines': len(self.df) if self.df is not None else 0,
